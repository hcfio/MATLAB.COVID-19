...
DENZL=zeros(DD,1);
for j=1:DD
    DENZL(j,1)=max(0,XNZL(j+891,1)-XNZL(j+884,1))/7;
end
...
DETWN=zeros(DD,1);
for j=1:DD
    DETWN(j,1)=max(0,UTWN(j+891,1)-UTWN(j+884,1))/7;
end
...
DETKY=zeros(DD,1);
for j=1:DD
    DETKY(j,1)=(TKY2(j+783,1)-TKY2(j+776,1))/7;
end
...
HKD=zeros(DD,1);
for j=1:DD
    HKD(j,1)=(HKD1(j+783,1)-HKD1(j+776,1))/7;
end
DEHKD=zeros(DD,1);
for j=1:DD
    DEHKD(j,1)=(HKD2(j+783,1)-HKD2(j+776,1))/7;
end
...
% plot
subplot(1,2,1)
plot([JPN,TKY,OKNW,OSK,HKD,NZL,KOR,HKG,TWN,AUS],'LineWidth',2)
title('COVID-19: 7-day average of new cases per 1M','data sourced by JHU and MoH of Japan')
xlabel('date');
ylabel('cases/1M');
xticks([1 63 DD])
xticklabels({[ll0],[ll2],[llf]})
legend('Japan','Tokyo','Okinawa','Osaka','Hokkaido','New Zealand','South Korea','Hong Kong','Taiwan','Australia','Location','north');
%
subplot(1,2,2)
plot([DEJPN,DETKY,DEOKNW,DEOSK,DEHKD,DENZL,DEKOR,DEHKG,DETWN,DEAUS],'LineWidth',2)
title('COVID-19: 7-day average of deaths per 1M','data sourced by JHU and MoH of Japan')
xlabel('date');
ylabel('deaths/1M');
xticks([1 63 DD])
xticklabels({[ll0],[ll2],[llf]})
legend('Japan','Tokyo','Okinawa','Osaka','Hokkaido','New Zealand','South Korea','Hong Kong','Taiwan','Australia','Location','northwest');
%
