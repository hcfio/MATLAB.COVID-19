...
% Kyushu code 41 42 43 44 45 46 47
PFUK=5.112176;
PSAG=0.805721;
PNAG=1.290992;
PKUM=1.722474;
...
KYU1=(VC(:,41)+VC(:,42)+VC(:,43)+VC(:,44)+VC(:,45)+VC(:,46)+VC(:,47))/PKYU;
KYU=zeros(DD,1);
for j=1:DD
    KYU(j,1)=(KYU1(j+783,1)-KYU1(j+776,1))/7;
end

MIY1=VC(:,46)/PMIY;
MIY=zeros(DD,1);
for j=1:DD
    MIY(j,1)=(MIY1(j+783,1)-MIY1(j+776,1))/7;
end
